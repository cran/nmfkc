...
```{r no-nlme, eval = !has_nlme, echo = FALSE, results = "asis"}
...
To obtain standard errors, z-values, p-values, and confidence intervals for
...
The sidedness follows the fit: sign-free $\Theta$ (`C.signed = TRUE`) uses a
...
$U A^\top = 0$: they are orthogonal to the covariate rows, so $U$ contains
exactly the individual variation that the covariates cannot explain. (When
$A$ includes an intercept row, as here, this implies the familiar
sum-to-zero centering of each row of $U$ across subjects.)
